  ...
  const getReports = async () => {
    try {
      const res = await axios.get(
        import.meta.env.VITE_URL_API + `reports/comment/sent/${farmerId}`
      );
      setReports(res.data.rows || []);
    } catch (err) {
      setReports([]);
      console.log("Error get my comment reports : ", err);
    }
  };
          ...
          <ul>
            ...
            <li>
              <a className="text-black">
                ประวัติการแจ้งรายงานความคิดเห็นของฉัน
              </a>
            </li>
          </ul>
        </div>
            ...
            <table className="table bg-base-100 w-full">
              <thead>
                <tr>
                  ...
                  <th className="text-center">สถานะความคิดเห็น</th>
                  <th className="text-center">เจ้าของความคิดเห็น</th>
                      ...
                      <td className="text-center">
                        <span
                          className={`badge badge-sm ${
                            item.comment_status === "ลบแล้ว"
                              ? "badge-error"
                              : "badge-success"
                          }`}>
                          {item.comment_status === "ลบแล้ว" ? "ซ่อน" : "แสดง"}
                        </span>
                      </td>
                      <td className="text-center">
                        <div className="max-w-xs">
                          <p
                            className="truncate"
                            title={item.comment_owner_name}>
                            {item.comment_owner_name}
                          </p>
                        </div>
                      </td>
                      <td className="text-center">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">
                      {selectedReport.comment_owner_name}
                    </p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <span
                      className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                        selectedReport.comment_status === "ลบแล้ว"
